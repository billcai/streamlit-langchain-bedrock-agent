...
class LLMThought:
    ...
    def on_llm_new_token(self, token: str, **kwargs: Any) -> None:
        ...
        self._llm_token_writer_idx = self._container.markdown(
            self._llm_token_stream, index=self._llm_token_writer_idx, unsafe_allow_html=True
        )
    ...
    def on_llm_end(self, response: LLMResult, **kwargs: Any) -> None:
        ...
        # data is redundant
        for gen in response.flatten()[0].generations:
            for single_gen in gen:
                if 'Thought:' not in single_gen.text:
                    generateText = 'Thought:'+single_gen.text
                else:
                    generateText = single_gen.text
                self._container.markdown(generateText)
        # if len(response.generations) >0 and len(response.generations[0]) > 0:
        #     firstRes = response.generations[0]
        #     for res in firstRes:
        #         textRes = res.text
        #         print(textRes)
        #         curIdx = 0
        #         self._container.markdown(
        #                 textRes, index=curIdx
        #             )
            # if 'Thought:' in textRes:
            #     textExtract = 'Thought:'+textRes.split('Thought:')[1]
            #     self._container.markdown(
            #         textExtract, index=curIdx
            #     )
            #     curIdx += len(textExtract)
            # if 'Action Input:' in textRes:
            #     textExtract = 'Action Input:'+textRes.split('Action Input:')[1]
            #     self._container.markdown(
            #         textExtract, index=curIdx
            #     )
            #     curIdx += len(textExtract)
    ...
    def on_tool_end(
        ...
        **kwargs: Any,
        ...
        self._container.markdown(f"**{output}**", unsafe_allow_html=True)
...
class StreamlitCallbackHandler(BaseCallbackHandler):
    ...
    def _prune_old_thought_containers(self) -> None:
        ...
        while (
            ...
            if self._history_container is not None:
                self._history_container.markdown(oldest_thought.container.label, unsafe_allow_html=True)
